...
Linux production execution may start from a normal unprivileged login identity with supplementary groups. Immediately after `clone3(CLONE_NEWUSER | CLONE_NEWPID | CLONE_NEWNS | CLONE_PIDFD)`, the namespace init uses its new-user-namespace capability to clear its inherited supplementary groups before it reports that the namespace is bound. The parent then disables further `setgroups` calls before it writes the one-entry GID map. The namespace init rechecks the empty group list after mapping, and the nested target bootstrap rechecks it before the credential and capability drop. The signed containment-helper receipt binds `namespace-cleared-supplementary-groups-v1` and an exact empty target group list. Failure to clear or verify the group list exits 125 before target code.

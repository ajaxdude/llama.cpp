...
A08 is terminal/no-retry. The executor released Strix after observing zero swap, a free port, zero containers/processes, and absent A08 paths, but llama-swap, llama-swap-proxy, and brainrouter were all inactive. The authoritative state is `RELEASED_BUT_DEGRADED`: unassigned but unavailable to PR60 until a separately authorized owner restores and verifies the repository-authorized services. Rootless execution cannot satisfy the current immutable-owner, supplementary-group, UID-map, and ROCm trust contract; the review is bound by SHA-256 `98a941861d93ce7ccafc76824c609d495fa7cb032c74b1b13006e8a9f711bb31`. Until the schema gate, reference manifest gate, native closure, and host restoration all pass, model-backed status is `INCOMPLETE`. No performance claim is authorized by model-free validation.
